...
const Map<String, String> ruStrings = {
  ...
  'backup.nowRunning': 'Создаём копию…',
  'backup.nowDone': 'Всё с этого телефона сохранено в резервной копии.',
  'backup.nowFailed':
      'Не удалось создать копию. Ничего не потеряно — сообщения по-прежнему на этом телефоне.',
  'backup.statusSafe':
      'Всё здесь сохранено в резервной копии. Потеря телефона не приведёт к потере этих сообщений.',
  'backup.statusPending': 'Сохраняем новые сообщения…',
  'backup.statusPendingCount':
      '{count} ещё не на сервере — они есть только на этом телефоне.',
  'backup.statusFailing':
      'Резервное копирование не работает. Новые сообщения есть только на этом телефоне.',
  'backup.statusNone':
      'Кода восстановления нет, поэтому ничего не сохраняется. Потеряете телефон — потеряете эти сообщения.',
  ...
  'recovery.restoreHere': 'Восстановить по коду',
  'recovery.restoreHereConfirm':
      'Устройство получит новую личность и загрузит историю из этой резервной копии. Сообщения, '
      'прочитанные только на этом устройстве и отсутствующие в копии, сохранятся. Отменить нельзя.',
  'recovery.restoreAction': 'Восстановить',
  'recovery.noBackup': 'Для этого аккаунта нет сохранённой резервной копии.',
  'backup.historyFromArchive':
      'Восстановлено из более ранней копии — в самой свежей не было истории сообщений.',
};
