...
Status: **design record with implementation notes.** The early decisions in this
document now underpin the implemented federation layer. For current operator and
protocol documentation, see [`../federation.md`](../federation.md).
...
- **F5 — MLS hub model.** *Not started — this is the centerpiece, and unlike
  ...
  The hub-migration ADR this called for is written:
  `docs/development/adr-federation-hub-migration.md`
  ...
  **Verified live (2026-07-21).** F5d was proven over the wire on two separately
  deployed hosts (one hub and one follower), each a full stack with its own
  Mongo, sharing a signed nodelist and
  authenticating S2S with Ed25519 over TLS. `alice@hub` adding
  `mimi://follower.example/u/<bob>` triggered an S2S mirror provision on the
